// 하나의 Java 파일에 클래스를 여러개 만드는 것과
// 각자 Java 파일로 클래스를 만드는 것이 모두 동일하게
// 서로 다른 클래스 파일로 만들어진다.
// bin 파일에서 class 파일 확인 가능

// Package : Java에서 Java파일과 class파일들을 폴더 단위로 관리하기 위한 목적으로 사용한다.
// Package에 포함되지 않은 파일들은 default package에 포함되어 있다고 한다.
// 다른 패키지에 있는 클래스들은 import를 사용하여 클래스가 어떤 패키지에 있는지 알려줘야 한다.

import com.test.pkg1.TestClass10;
import com.test.pkg1.TestClass11;

// *를 이용하면 지정된 패키지의 모든 클래스를 import 할 수 있다.
// 불필요한 클래스를 import 하게 되므로 비효율적일 수 있다.
// 코드를 작성할 때 불편해질 수 있다.
import com.test.pkg2.*;

// 위에서 TestClass10은 com.test.pkg1에 있다고 명했기 때문에
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// import 된 다른 패키지의 클래스와 동일한 이름의 클래스를 사용할 때는
		// 앞에 패키지 명을 반드시 붙여줘야 한다.

fun main() {
    ...
    // compareTo 는 문자열을 구성하는 각 글자의 코드 값을 합산한 결과를 뺀 값을 전달한다.
    println(str3.compareTo(str4))
    println(str3.compareTo(str5))

    // 두 번째 매개변수에 true를 넣어주면
    // 모두 소문자로 변환한 다음 코드 값을 빼는 작업을 수행한다.
    ...
    // String은 문자열 합치는 작업을 수행할 때마다 새로운 문자열 객체가 계속 생성된다.
    // (String은 관리하는 문자열 수정이 불가능하기 때문에)

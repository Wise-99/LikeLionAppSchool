fun main() {
    ...
    // 관리할 객체들(값들)이 있고, 나중에 추가, 삭제 등을 하지 않고 관리하는 값을 변경하지 못하게 관리하고자 할 때 : listOf

    println("---------------------------------------------")
    // 관리하는 객체(값)에 접근
    //[ ] 연산자를 통해 순서 값(0부터 1씩 증가)을 지정하여 접근한다.
    ...
    // 순서값 3(네번째)에 지정된 요소가 관리하는 값들을 모두 추가하고 그 이후의 값들은 뒤로 밀린다.

fun main() {
    // 코틀린 범위 지정 함수
    // 객체를 생성할 때 파라미터에 값을 설정하는 것과 관련이 있다.
    ...
    // 만약 아래와 같이 객체 생성 시 프로퍼티의 값을 저장하고 객체의 ID를
    ...
    println("-------------------------------------")
    // apply : 생성된 객체의 ID가 apply쪽으로 전달되어
    ...
    // this(혹은 생략)을 통해 프로퍼티에 접근할 수 있다.
    // run { } 블럭의 수행이 끝나면 제일 마지막에 작성한 값이 변환되어 변수에 담긴다.
    // 이에 생성된 객체의 ID를 전달하고자 한다면 run{ } 블럭 제일 아래에 this를 작성하여
    // 객체의 ID를 반환해야 한다.

package com.test.android87_compass
...
import com.test.android87_compass.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val accListener = object : SensorEventListener{
            override fun onSensorChanged(event: SensorEvent?) {
                ...
                accValue[0] = event?.values?.get(0)!!
                accValue[1] = event?.values?.get(1)!!
                accValue[2] = event?.values?.get(2)!!
        ...
        val magListener = object : SensorEventListener{
            override fun onSensorChanged(event: SensorEvent?) {
                // 자기장 센서로 측정된 값을 담아준다.
                magValue[0] = event?.values?.get(0)!!
                magValue[1] = event?.values?.get(1)!!
                magValue[2] = event?.values?.get(2)!!
    ...
    fun getAzimuth(){
        ...
        if(isGetAcc == true && isGetMag == true){
            ...
            // 이미지 뷰 회전
            activityMainBinding.imageView.rotation = (360 - azimuth).toFloat()
        }
    }
}
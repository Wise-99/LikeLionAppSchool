...
// 안드로이드는 AndroidManifest.xml에 권한 사용을 등록하면 사용할 수 있었지만
// 개인 정보 문제로 인해 사용자에게 확인받아야하는 것으로 변경되었다.
...
class MainActivity : AppCompatActivity() {
    ...
    // 모든 권한에 대해서 한번에 요청하고자 한다면 requestPermission을 사용하고
    // 권한 확인 후에 처리가 필요하다면 onRequestPermissionsResult 메서드를 overriding하고
    // 권한 별로 분기하여 처리하면 된다.
    // 만약, 권한 요청 후 필요한 처리를 권한 별로 나눠서 구현하고 싶다면 ActivityResultCallback을 사용한다.

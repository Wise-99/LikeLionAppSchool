package com.test.android72_sqlitedatabase1
...
class DBHelper(context: Context) : SQLiteOpenHelper(context, "Test.db", null, 1) {
    // 데이터베이스 파일이 없으면 만들고 이 메서드를 호출해준다.
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        // autoincrement : 커럼에 저장할 값을 지정하지 않으면 1부터 1씩 증가되는 값이 자동으로 저장한다.
    ...
    // 사용하는 데이터베이스 파일의 버전이 변경되어 있을 떼 호출되는 메서드
